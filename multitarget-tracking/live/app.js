...
function initialState(){return {selected:0,playing:false,visible:true}}
...
function currentView(state){const item=cases[state.selected];return {...item,src:'../assets/'+(state.playing&&state.visible?item.animation:item.still),playing:state.playing&&state.visible}}
...
function render(){const v=currentView(state);if(picture.getAttribute('src')!==v.src)picture.src=v.src;picture.alt=`Original nuScenes visualization: ${v.title}. ${v.observation}`;document.querySelector('#observation').textContent=v.observation;document.querySelector('#watch').textContent=v.watch;play.textContent=v.playing?'Stop animation':'Play original animation';play.setAttribute('aria-pressed',String(v.playing));document.querySelector('#source').href='https://www.notion.so/2270d664735d8183bd8bc130601992e2#'+v.source;document.querySelector('#full-image').href=v.src;scene.value=state.selected;document.querySelector('#status').textContent=v.playing?'Original animation playing':'Still image';}
