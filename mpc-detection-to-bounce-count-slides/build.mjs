...
slides.push(regular(
  ...
  [
    ...
    image(
      ...
      {
        fit: 'contain',
        alt: 'Frameless oblique 3D scene with a base-station antenna, green user equipment standing clear of the right wall, and specular direct, one-bounce, and two-bounce radio routes.',
        fx: { enter: 'fade-up', order: 2 }
      }
    ),
    // Image-source solutions on the visible wall-face tangents.
    // Foreground: (960.29, 403.10)→(1048.94, 358.57); rear: (899.15, 308.79)→(1040.90, 240.07).
    // The rounded coordinates retain |cos θᵢ − cos θᵣ| < 3e-7 at every bounce.
    line('route-los-glow', 890, 318, 1061, 315, C.measurement, 9, { opacity: .18 }),
    line('route-single-a-glow', 890, 318, 1043.3253, 361.3903, C.map, 9, { opacity: .18 }),
    line('route-single-b-glow', 1043.3253, 361.3903, 1061, 315, C.map, 9, { opacity: .18 }),
    line('route-double-a-glow', 890, 318, 989.5002, 388.4273, C.known, 9, { opacity: .18 }),
    line('route-double-b-glow', 989.5002, 388.4273, 992.4799, 263.5439, C.known, 9, { opacity: .18 }),
    line('route-double-c-glow', 992.4799, 263.5439, 1061, 315, C.known, 9, { opacity: .18 }),
    line('route-single-normal', 1038.84, 352.45, 1047.81, 370.33, C.faint, 1, { opacity: .7 }),
    line('route-double-normal-a', 985.01, 379.49, 993.99, 397.36, C.faint, 1, { opacity: .7 }),
    line('route-double-normal-b', 988.12, 254.55, 996.84, 272.54, C.faint, 1, { opacity: .7 }),
    line('route-los', 890, 318, 1061, 315, C.measurement, 3),
    line('route-single-a', 890, 318, 1043.3253, 361.3903, C.map, 3),
    line('route-single-b', 1043.3253, 361.3903, 1061, 315, C.map, 3),
    line('route-double-a', 890, 318, 989.5002, 388.4273, C.known, 3),
    line('route-double-b', 989.5002, 388.4273, 992.4799, 263.5439, C.known, 3),
    line('route-double-c', 992.4799, 263.5439, 1061, 315, C.known, 3),
    shape('route-single-point', 1038.3253, 356.3903, 10, 10, C.paper, { shape: 'ellipse', stroke: C.map, strokeWidth: 3 }),
    shape('route-double-point-a', 984.5002, 383.4273, 10, 10, C.paper, { shape: 'ellipse', stroke: C.known, strokeWidth: 3 }),
...
for (const unit of sectionUnits) {
  ...
  if (unit.id === 'pose') {
    slides.push(regular(
      ...
      'Formulate the single-snapshot corner case before opening the interactive construction. The BS pose and clock synchronization are known. Path 1 reflects on wall A; path 2 follows A→B. Both delay/AoD/body-AoA tuples are noisy, and there is deliberately no LoS factor. Emphasize that the local measurement Jacobian is 6×7, so a generic solution has at least one null direction; noise turns the exact continuum into a likelihood ridge rather than a unique estimate.',
      [
        ...
        text('pose-double-measurements-eq', 116, 286, 290, 92, texBlock`\begin{aligned}\tilde{\mathbf z}_k&=[c\tilde\tau_k,\,\tilde\psi_k,\,\tilde\varphi_k^{\rm b}]^{\mathsf T}\\&=\mathbf h_k(\mathbf x)+\boldsymbol\epsilon_k,\quad k\in\{1,2\},\\\boldsymbol\epsilon_k&\sim\mathcal N(\mathbf 0,\mathbf R_k).\end{aligned}`, 14, { color: C.ink, align: 'center' }),
        text('pose-double-measurements-copy', 118, 384, 286, 34, 'k = 1 is the single-bounce MPC; k = 2 is the double-bounce MPC.', 12, { color: C.soft, fontFamily: SANS, lineHeight: 1.35 }),
        ...
        text('pose-double-angle-eq', 462, 490, 336, 54, texBlock`\begin{aligned}\psi_k&=\angle(\mathbf q_{k,1}-\mathbf b)-\theta_B,\\\varphi_k^{\rm b}&=\operatorname{wrap}[\angle(\mathbf q_{k,\rm last}-\mathbf p)-\theta].\end{aligned}`, 12.5, { color: C.ink, align: 'center' }),
        ...
        text('pose-double-conclusion', 118, 616, 1044, 28, 'Noise thickens the remaining null curve. The next interactive slide visualizes feasible slices and rejects non-forward or negative-length routes.', 15, { color: C.paper, fontFamily: SANS, fontWeight: 700, align: 'center', valign: 'middle' })
