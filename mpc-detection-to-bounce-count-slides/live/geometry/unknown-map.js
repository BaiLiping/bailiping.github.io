...
if(new URLSearchParams(location.search).get("construction")!=="incidence-points"){
...
function render(){
  ...
  const solutionFeasible=entryFeasible&&!!Xraw&&Number.isFinite(m2Solved)&&m2Solved>0&&m2Solved<rem&&dist(U1,Xraw)<1e-5;
  ...
  $("stat32").innerHTML= step<5 ? "" :
    (!entryFeasible?`<span class="off">✗ rejected (P⁽¹⁾, θ): path 2 reaches wall A behind the BS or after exhausting L₂.</span>`:
     ...
     !solutionFeasible?`<span class="off">✗ rejected (P⁽¹⁾, θ): the crossing fails the forward-segment or shared-UE closure test. A line crossing is not a physical bounce.</span>`:
...
function render(){
  ...
  const doubleFeasible=doubleEntry&&!!X2raw&&Number.isFinite(m2)&&m2>0&&m2<rem2&&dist(U1,X2raw)<1e-5;
  ...
  function strip3(pp){
    ...
    const feasible=!!X3raw&&Number.isFinite(m3)&&m3>0&&m3<rem3&&dist(U1,X3raw)<1e-5;
    return {R1,R2,E3,e32,scale,line3,X3:feasible?X3raw:null,m3,rem3,feasible,reason:feasible?"":"the line crossing fails forward length or shared-UE closure"};
  }
  const S=strip3(p3);
  ...
  $("stat33").innerHTML= step<3 ? "" :
    (!doubleFeasible?`<span class="off">✗ rejected (P⁽¹⁾, θ): the two-bounce prefix violates ray order, its delay budget, or shared-UE closure.</span>`:
...
/* ---- section 4.5: unknown UE, corridor triple — enforce repeated-wall closure ---- */
(function(){
const svg=$("svg35");
if(!svg)return;
...
const CAP=["",
 ...
 "④ <b>Reuse R.</b> The third point must lie on the original wall R and reflect toward the same UE. Coincident candidate lines alone cannot establish that.",
 "⑤ <b>Test, do not assume, the ambiguity.</b> Moving the first-bounce or heading hypothesis can now fail original-wall incidence or reflection. This is an exact specular consistency test, not an optimizer for noisy or diffuse paths."];
let step=1;
function goStep(n){
...
function render(){
  ...
  function strip3(pp){
    ...
    // The route is R -> L -> the SAME R: never invent a third wall.
    const P3=[R2[0]+m3*e32[0],R2[1]+m3*e32[1]];
    const wallError=Math.abs((P3[0]-Pw[0])*nA[0]+(P3[1]-Pw[1])*nA[1]);
    const incidence=e32[0]*nA[0]+e32[1]*nA[1];
    const outgoing=[e32[0]-2*incidence*nA[0],e32[1]-2*incidence*nA[1]];
    const reflectionError=Math.hypot(outgoing[0]+pp.u[0],outgoing[1]+pp.u[1]);
    const forward=Number.isFinite(m3)&&m3>0&&m3<rem3&&off<1e-5;
    const feasible=forward&&wallError<1e-5&&reflectionError<1e-7;
    const reason=!forward?"shared UE or positive final-leg closure failed":
      wallError>=1e-5?"third bounce misses the ORIGINAL wall R":
      reflectionError>=1e-7?"third bounce violates reflection at ORIGINAL wall R":"";
    return {R1,R2,E3,line3,e32,rem3,m3,off,P3,wallError,reflectionError,feasible,reason};
  }
  const S=strip3(p3);
  ...
  if(step>=4&&S.feasible){
    out+=arrow(S.R2,S.P3,"#b45607",2)+arrow(S.P3,U1,"#b45607",2);
    out+=`<circle cx="${S.P3[0]}" cy="${S.P3[1]}" r="5" fill="#e8720c"/>`+txt(S.P3[0]+9,S.P3[1]-8,"P₃ on ORIGINAL R","#b45607",10);
    ...
    out+=txt(line1.o[0]+430*line1.v[0],line1.o[1]+430*line1.v[1]+16,Math.abs(dH)<0.05?"line 3 ≡ line 1":"line 3: also satisfies the original-R test","#b45607",10);
  }else if(step>=4&&S.R1){
  ...
  $("stat35").innerHTML= step<4 ? "" :
    ...
    !S.feasible?`path-2 prefix ✓ · <span class="off">path 3 rejected: ${S.reason}.</span>`:
    `same UE ✓ · positive segments ✓ · ORIGINAL wall R ✓ · wall error = <b>${(S.wallError*M_PER_PX).toExponential(1)} m</b> · reflection error = <b>${S.reflectionError.toExponential(1)}</b>. This slice passes the repeated-wall test; not every slider setting does.`);
}
["s35P","s35H","s35L","s35A","s35D","sS35"].forEach(id=>$(id).addEventListener("input",render));
function resetRepeatedWall(){
  ["s35L","s35A","s35D","s35H","sS35"].forEach(id=>$(id).value=0);
  $("s35P").step="any";
  $("s35P").value=(D7.RX-D7.BS[0])/(2*D7.RX-D7.BS[0]-D7.UE[0]);
}
$("b35R").addEventListener("click",()=>{resetRepeatedWall();render();});
