...
const outputPath = resolve(here, 'index.html');
...
const aliases = {
  main: 'overview',
  objective: 'elbo',
  factorization: 'mean-field',
  demo: 'em',
  'em-demo': 'em'
};
const routeIndexes = Object.fromEntries(
  Object.entries({ ...slideIndexes, ...aliases }).map(([route, target]) => [
    route,
...
const pageTitle = '<title>Variational Inference: Approximation by Optimization | Bai Liping</title>';
const pageDescription = '<meta name="description" content="An interactive Bento deck on ELBOs, mean-field inference, coordinate ascent, EM, stochastic gradients, and variational diagnostics." />';
const canonical = '<link rel="canonical" href="https://bailiping.com/variational-inference/" />';
const liveStylesheet = '<link rel="stylesheet" href="../assets/bento-inline-live.css?v=2" />';
...
html = html.replaceAll('../assets/bento-live.css', '../assets/bento-inline-live.css?v=2');
html = html.replaceAll('../assets/bento-live.js', '../assets/bento-inline-live.js');
html = html.replace(
  '<!DOCTYPE html>',
  '<!DOCTYPE html>\n<!-- Generated by variational-inference/build-bento.mjs. Edit bento-deck.mjs, then rebuild. -->'
);
html = html.replace(/[ \t]+$/gm, '').replace(/\n*$/, '\n');

if (!html.includes('"docId": "variational-inference-bento"')) throw new Error('Bento document replacement failed');
...
if (ids.size !== deck.slides.length) throw new Error('Duplicate slide IDs found');
for (const entry of inlineLiveMap) {
  const liveIndex = slideIndexes[entry.slide];
  if (entry.slideIndex !== liveIndex) throw new Error(`Incorrect slideIndex for ${entry.slide}`);
  if (liveIndex < 1 || deck.slides[liveIndex - 1].id !== entry.introSlide) {
    throw new Error(`${entry.slide} must immediately follow ${entry.introSlide}`);
  }
}
