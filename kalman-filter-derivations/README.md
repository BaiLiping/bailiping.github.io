...
The public reference and slide views use the same content.

- Edit `index.html` for equations, copy, references, and slide order.
- Edit `consolidated.css` and `consolidated.js` for layout and navigation.
- Run `node kalman-filter-derivations/build-consolidated.mjs` from the repository root to synchronize `consolidated-slides.html`.
- `slides.html` redirects to the consolidated presentation.
