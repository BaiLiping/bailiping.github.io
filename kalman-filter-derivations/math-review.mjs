...
export function applyMathReview(slides, helpers) {
  ...
  after('bayes-equations', sheet({
    ...
    panels: [
      ...
      { title: 'Bridge back to information form', body: paras(
        ...
        muted(`The first line is Woodbury. For the second, multiply by ${tex`J`} and use ${tex`S=HPH^\mathsf T+R`}.`)
      ), fontSize: 15.2 },
  ...
  edit('mse-live', 'geometry-metric-copy', `Covariance reduction follows ${tex`P^-h`}, not generally ${tex`h`}.`, { fontSize: 15.5 });
  ...
  edit('least-squares-equations', 'least-squares-equations-panel-4-body', lines(
    ...
    muted(`Errors are zero-mean and uncorrelated, with covariances ${tex`P^-`} and ${tex`R`}.`)
  ...
  after('implementations-live', sheet({
    ...
    panels: [
      ...
      { title: 'Singular priors are not invalid priors', body: paras(
        ...
        muted(`Do not use ${tex`(P^-)^{-1}`}, a log determinant, or an ordinary density on the full space without adapting the representation.`)
      ), fontSize: 15 },
