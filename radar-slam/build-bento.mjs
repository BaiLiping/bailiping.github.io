...
html=html.replace(/<title>[\s\S]*?<\/title>/,'<title>Radar SLAM, one scan at a time | Bai Liping</title>');
html=html.replace(/<meta name="description"[^>]*>/,`<meta name="description" content="${deck.slides.length} Bento slides with seven live radar SLAM experiments: measurements, Doppler, scan registration and loop-closure optimization." />`);
html=html.replace(/<link rel="canonical"[^>]*>/,'<link rel="canonical" href="https://bailiping.com/radar-slam/" />');
html=html.replace(/<!-- Generated by kalman-filter-derivations[^>]*-->/,'<!-- Generated by radar-slam/build-bento.mjs. Edit bento-deck.mjs, then rebuild. -->');
// Natural-height display spans keep grouped equations inside their native panels.
html=html.replace(/<link[^>]+href="visual\.css[^"]*"[^>]*>/,'');
html=html.replace('</head>',`<style id="radar-math-layout">
