(() => {
  ...
  const trajectory = window.HandoverTrajectory
  const MAX_FRAME = trajectory.maxFrame
  const FRAME_MS = 125
  const SCALE = 1.9
  const ORIGIN_X = 255
  const ORIGIN_Y = 236
  const FOV_RADIUS = trajectory.fovRadius
  ...
  const targetElements = Object.fromEntries(targets.map((target) => [target.id, {
    route: document.querySelector(`.route-${target.id.toLowerCase()}`),
  ...
  function positionAt(target, frame) {
    return trajectory.positionAt(target.id, frame)
  }
  ...
  function renderTrajectories() {
    targets.forEach((target) => {
      const path = Array.from({ length: MAX_FRAME + 1 }, (_, frame) => {
        const point = screenPoint(positionAt(target, frame))
        return `${frame === 0 ? 'M' : 'L'}${point.x.toFixed(2)} ${point.y.toFixed(2)}`
      }).join('')
      targetElements[target.id].route.setAttribute('d', path)
    })
  }
  ...
  function renderTransport() {
    ...
    elements.timeline.style.setProperty('--timeline-progress', `${state.frame / MAX_FRAME * 100}%`)
  ...
  document.addEventListener('keydown', (event) => {
    // The frame adapter owns presentation navigation when embedded.
  ...
  renderTrajectories()
  elements.timeline.max = String(MAX_FRAME)
  enableControls()
  elements.existenceOutput.value = Number(elements.existenceThreshold.value).toFixed(2)
  elements.visibilityOutput.value = Number(elements.visibilityThreshold.value).toFixed(2)
  rebuildEvents()
