// Idempotent integration. Keep the original and reviewed equations intact.
...
- \`visual-polish.mjs\`: native Bento layout, cover, comparison table, route summaries, and reading progress. Applied after the content and mathematical-review layers; does not change algorithms.
- \`visual.css\`: deck-scoped finishing and coordinated light-theme experiment controls.
- \`live/families.js\`: pure numerical models shared by browser demos and tests.
...
Four derivation principles: probability/conditioning, second-moment projection, quadratic objectives, and variational/KL updating. Two computational viewpoints: recursive elimination/messages, and numerical representations/algebraic bridges. The catalogue lists 30 principal routes, reformulations, and extensions, not 30 independent filters or an exhaustive taxonomy. Unrestricted KL updating is Bayes, and QR is an implementation rather than another statistical optimality principle.

The opening slides compare questions, unknowns, governing equations, outputs, and overlap. Each group has a route summary and a distinctive experiment:

- Bayes: Gaussian versus mixture prior with equal first two moments; full posterior versus the Kalman Gaussian.
...
- KL: candidate mean/variance, entropy, evidence, and exact free-energy gap.
- Recursive inference: stepwise Schur elimination; batch, filtering, and RTS smoothing.
- Numerical forms: retained finite-precision comparison of covariance, information, Joseph, and QR.

The original scalar-fusion and covariance-geometry labs remain. Every lab has a static/print fallback. Existing named deep links remain available; every slide links back to the group map. The earlier legacy deck is unchanged.

## Visual system

Warm paper, high-contrast ink, Georgia display headings, system sans-serif text, and a consistent accent per group. Equation rows have dedicated rendering space. The six-card opening separates principles from computational views; the progress line tracks position. No new font or image download is needed. New controls support keyboard navigation, mobile stacking, and reduced-motion preferences.
`);
console.log('Integrated six viewpoints, five new experiments, and the visual finishing layer.');
