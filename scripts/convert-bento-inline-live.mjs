...
const decks = [
  ...
  },
  {
    ...
    bounds: { x: 40, y: 178, width: 1200, height: 486 },
    demos: [
      {
        state: 'state-handover-rule-live',
        introSlide: 's-criterion',
        slide: 's-criterion-live',
        title: 'Point-target handover: gates, timing, and messages',
        source: './live/',
        src: './live/?embed=region&v=20260918-trajectory'
      }
    ]
  }
]

function mountMarker(bounds) {
  return {
    id: 'live-demo-mount',
    type: 'shape',
    shape: 'rect',
...
function convertDeck(definition) {
  const deckBounds = definition.bounds || bounds
  ...
  const liveMap = definition.demos.map(demo => ({
    ...(demo.introSlide ? { introSlide: demo.introSlide } : {}),
    slide: demo.slide,
