...
const expectedSourceHash = 'd70751dd775012ead5d55adc9c93433256f2c5751f07dc96fff3c96a8149ca3e';
...
function refs(keys) {
  return keys.map(key => {
    ...
    return '<a href="' + esc(ref.url) + '" target="_blank" rel="noopener noreferrer" title="' + esc(ref.title) + '">' + key + '</a>';
  }).join('');
}
const ids = new Set();
...
const html = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
  ...
  '<button data-full>Full screen · F</button><a href="slides.pdf" target="_blank" rel="noopener">PDF</a><button data-print>Print</button></nav>' +
