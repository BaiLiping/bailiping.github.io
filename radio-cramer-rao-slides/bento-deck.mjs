...
two('project-setup','Configured radio experiment','The physical coded-pilot observation','One BS array transmits 50 known spatial codes. Every received symbol is retained.',
 'SIGNAL AND RECEIVER CLOCK',M(R`\mathbf Y_k=\widetilde{\mathbf H}_k\mathbf X+\mathbf W_k`)+M(R`\widetilde{\mathbf H}_k=e^{-j2\pi f_k\beta}\mathbf H_k^{\mathrm g}`)+M(R`\mathbf X\in\mathbb C^{384\times50},\quad\mathbf Y_k\in\mathbb C^{32\times50}`)+'The same X is reused on every tone. β is clock bias in seconds.',
 'CONFIGURED DIMENSIONS','<b>BS:</b> 16 × 24 antenna ports.<br><b>UE:</b> 4 × 8 antenna ports.<br><br><b>50 symbols, 3300 tones, 400 MHz.</b><br>Carrier: 27.2 GHz. Spacing: λ/2.<br><br>'+M(R`f_k=(k-\lfloor K/2\rfloor)B/K`)+ 'The tensor contains 5,280,000 complex observations per acquisition.',
 'The system model calls the symbol count S and tone count N. This deck uses L and K for the same quantities. The configured matrices have 384 TX and 32 RX ports on one BS and one UE. No spatial scene, trajectory, or saved RT realization is embedded in this teaching demo. The frequency grid includes DC and is not exactly centered for even K. The fixed physical code matrix is reused at every bin.',['Model','Li'],[23,21]);
...
add('project-fim','Configured radio experiment','The actual pilots enter the Fisher matrix','Compressed training can identify a structured path even when arbitrary full CSI is unavailable.',[
 ...panel('general-fim',96,190,1088,211,'JOINT INFORMATION FROM THE RECEIVED I/Q',M(R`\mathbf M_k=e^{-j2\pi f_k\beta}\mathbf H_k^{\mathrm g}(\boldsymbol\eta)\mathbf X`)+M(R`J_{ij}=\frac{2}{\sigma^2}\operatorname{Re}\sum_k\operatorname{tr}\!\left[(\partial_i\mathbf M_k)^H(\partial_j\mathbf M_k)\right]`),{size:23,fill:C.cool}),
...
],'For any real parameter eta_i, partial_i M_k = exp(-j2pi f_k beta)[partial_i H^g_k - j2pi f_k H^g_k partial_i beta] X. A fixed covariance sigma²I gives J=2/sigma² Re(D^H D). The rank relevant for local parameter identifiability is the realified stacked derivative matrix. A pseudoinverse of XX^H recovers only the excited transmit subspace. Free path gains and geometric delays plus an unconstrained common clock create a gauge that transmit diversity alone cannot remove.',['Model','Li','LeMagoarou']);
...
two('project-codes','Configured radio experiment','A coded-pilot bounds calculator','The next lab evaluates an isolated representative path using the physical observation design.',
 'POWER AND NOISE',M(R`X_{ps}=\sqrt{\frac{P}{N_tK}}\,C_{ps}`)+M(R`C_{ps}\in\{1,j,-1,-j\}`)+M(R`\sigma^2=10^{(-174+\mathrm{NF}-30)/10}\Delta f`)+ 'At P = 55 dBm and NF = 9 dB:<br>σ² = 3.8331 × 10⁻¹⁵ W per complex sample.',
...
two('known-channel-clock','Known geometric channel','Known propagation can leave an unknown clock','The geometric channel provides a template with an absolute delay reference.',
 'MEAN AND CLOCK DERIVATIVE',M(R`\mathbf A_k=\mathbf H_k^{\mathrm g}\mathbf X`)+M(R`\mathbf M_k=e^{-j2\pi f_k\beta}\mathbf A_k`)+M(R`\partial_\beta\mathbf M_k=-j2\pi f_k\mathbf M_k`)+ 'The receiver compares this template with noisy received samples.',
 'CLOCK INFORMATION',M(R`w_k=\|\mathbf A_k\|_F^2`)+M(R`J_{\beta\beta}=\frac{8\pi^2}{\sigma^2}\sum_k f_k^2w_k`)+M(R`\sigma_{\beta,\mathrm{LB}}=J_{\beta\beta}^{-1/2}`)+ 'This form assumes the complex phase reference is also known.',
...
add('known-channel-phase','Known geometric channel','Receiver phase changes the clock bound','An unknown common phase ψ is estimated jointly with the clock bias β.',[
 ...panel('phase-schur',96,190,1088,178,'INFORMATION AFTER ELIMINATING THE COMMON PHASE',M(R`J_{\beta,\mathrm{eff}}=\frac{8\pi^2}{\sigma^2}\sum_k w_k(f_k-\bar f_w)^2,\qquad\bar f_w=\frac{\sum_kw_k f_k}{\sum_kw_k}`),{size:23,fill:C.cool}),
 ...panel('weighted-spread',96,391,532,248,'ENERGY AND FREQUENCY SPREAD',M(R`\Gamma=\frac{\sum_kw_k}{\sigma^2}`)+M(R`B_{\mathrm{eff}}^2=\frac{\sum_kw_k(f_k-\bar f_w)^2}{\sum_kw_k}`),{size:22}),
 ...panel('clock-bound',652,391,532,248,'CLOCK STANDARD-DEVIATION BOUND',M(R`\sigma_{\beta,\mathrm{LB}}=\frac{1}{2\pi B_{\mathrm{eff}}\sqrt{2\Gamma}}`)+ 'At fixed energy, concentrating the received signal in a narrow part of the band raises the timing bound.',{size:22,fill:C.mint})
],'This is the Schur complement of the two-parameter beta,psi Fisher matrix. The weighted frequency mean need not vanish even on an arithmetically centered grid. Under a flat energy spectrum, K=3300, B=400MHz and aggregate Gamma=1000 give B_eff=115.470MHz and a clock standard-deviation lower bound of 30.82ps. Those are illustrative energy assumptions, not a numerical result from a saved RT run. A single nonzero tone with unknown phase cannot identify clock delay.',['Li','Shah']);
...
two('known-channel-lab','Known channel experiment','What does exact channel knowledge remove?','The next lab separates a timing benchmark from geometric identifiability.',
 ...
 'THE DELAY–CLOCK AMBIGUITY',M(R`\tau_\ell^{\mathrm a}=\tau_\ell^{\mathrm g}+\beta`)+M(R`(\tau_\ell^{\mathrm g}+\delta)+(\beta-\delta)=\tau_\ell^{\mathrm a}`)+ 'With free path gains and no geometric constraint, changing both terms this way preserves the effective channel.',
 'Knowing effective Htilde differs from knowing the geometric template H^g. Exact Htilde alone does not establish an absolute geometric delay origin if clock bias is free. The interactive controls show the invariant sum. For the oracle clock bound, H^g and its delay origin are instead given, so beta can be identifiable locally. Uniform tones retain the usual global delay ambiguity period 1/Deltaf.',['Model','Li','Shah'],[22,23]);
...
add('derivatives','05 · the derivatives','Parameter changes in the received signal','For one separated TX–RX coefficient, write its noise-free mean as μ.',[
 ...
 ['Path phase φ',I(R`\partial_\phi\mu=j\mu`),'Common phase change'],
