...
add('overview','Radio detection / precision limits','Cramér–Rao bounds for radio measurements','How much information does a MIMO–OFDM experiment contain?',[
 txt('hero',96,208,1088,118,'Delay, angles, and path gain.<br><span style="color:#0A6B5E">A bound you can calculate.</span>',{fontFamily:serif,fontSize:48,fontWeight:700,lineHeight:1.1}),
 txt('intro',96,352,1010,62,'Specify the pilots, the arrays, and the noise. Inspect how a change in each unknown changes the received I/Q data.',{fontSize:23,color:C.muted}),
 ...[['01','Bounds calculator','calculator-live',C.cool],['02','Fisher information','fisher-live',C.mint],['03','Bandwidth & time','bandwidth-live',C.warm],['04','3D arrays','geometry-live',C.cool]].flatMap(([n,t,link,fill],i)=>[box('tile-'+i,96+278*i,458,254,118,fill),txt('nav-'+i,117+278*i,479,212,76,n+'<br><b>'+t+'</b>',{fontSize:20,link})]),
...
],'A public educational companion to the existing radio multipath geometry presentation. One BS array and one UE array. Static, isolated far-field path with calibrated hardware and balanced orthogonal pilots. Five interactive labs also include multipath gain separability.',['Li','Shah','Background']);
...
add('signal','02 · the radio likelihood','Known pilots, unknown channel','All seven parameters appear in the mean of the received complex samples.',[
...
for(const entry of liveMap)slides.find(s=>s.id===entry.slide).elements.push(txt('open-lab',760,657,240,18,'OPEN FULL-SIZE LAB ↗',{fontSize:11,fontFamily:mono,color:C.blue,link:'https://bailiping.com/radio-cramer-rao-slides/live/?lab='+entry.src.match(/lab=([^&]+)/)[1]}));
