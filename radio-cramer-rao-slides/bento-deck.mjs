...
add('overview','Radio detection / precision limits','Cramér–Rao bounds for radio measurements','How much information does a MIMO–OFDM experiment contain?',[
 txt('hero',96,208,1088,118,'Known pilots. Unknown channel.<br><span style="color:#0A6B5E">What can noisy I/Q reveal?</span>',{fontFamily:serif,fontSize:44,fontWeight:700,lineHeight:1.1}),
 txt('intro',96,349,1088,62,'Bound delay, AoA, AoD and path gain from 384-port coded pilots. The channel and the receiver clock are estimated, never given.',{fontSize:22,color:C.muted}),
 txt('update',96,420,1088,22,'UPDATED 24 SEPTEMBER 2026 · 384-PORT CODED PILOTS · UNKNOWN CHANNEL AND CLOCK',{fontSize:12,fontFamily:mono,fontWeight:700,color:C.orange}),
 ...[['01','384 × 50 pilots','project-live',C.cool],['02','Unknown channel','unknown-channel',C.mint],['03','Reference model','calculator-live',C.warm],['04','Fisher information','fisher-live',C.cool]].flatMap(([n,t,link,fill],i)=>[box('tile-'+i,96+278*i,458,254,118,fill),txt('nav-'+i,117+278*i,479,212,76,n+'<br><b>'+t+'</b>',{fontSize:20,link})]),
...
two('meaning','01 · what a bound means','The CRB describes estimation precision','A measurement is an estimate extracted from noisy radio samples.',
 ...
 'WHAT THE NUMBER MEANS','A delay bound of <b>0.03 ns</b> is a lower bound on standard deviation, evaluated at the true channel. The estimator does not know that channel.<br><br>It describes one local solution. Detection failures, ambiguous solutions, bias, and model mismatch can make actual errors much larger.',
...
add('unknown-channel','Unknown channel','The receiver knows its pilots, not the channel','It sees noisy I/Q and the X it transmitted. Every propagation parameter must be estimated.',[
 table('knowledge',['Quantity','Known to the estimator?','Role in the bound'],[
 ['Pilots '+I(R`\mathbf X`)+', tone grid '+I(R`f_k`)+', carrier','Yes','Transmitted by design. They shape every derivative.'],
 ['Array geometry, calibration, noise variance σ²','Yes (assumed)','Steering vectors and noise weighting.'],
 ['Per path: '+I(R`\tau^{\mathrm a}_\ell`)+', AoA, AoD, '+I(R`\ell_\ell`)+', '+I(R`\phi_\ell`),'No','Estimated jointly. The CRB bounds these.'],
 ['Receiver clock bias '+I('b'),'No','Enters only through '+I(R`\tau^{\mathrm a}_\ell=\tau^{\mathrm g}_\ell+b`)+'.'],
 ['Number of paths P','No','Detection and model order. The CRB is conditional on P.'],
 ['True parameter values','Only to the simulator','The point where the CRB is evaluated. Never an estimator input.']
 ],{fontSize:18,widths:[1.45,.95,1.9],h:432})
],'The deterministic CRB is a function of the true parameters, so a simulator evaluates it at the path it generated. The estimator never receives that path: it receives Y and X and must estimate delay, both angle pairs, attenuation and phase of every path, with the clock bias entering through each apparent delay. The coded-pilot lab evaluates this unknown-channel CRB at one representative path. The number of paths is a detection and model-order question outside the CRB.',['Model','Li','LeMagoarou']);

add('unknown-clock','Unknown channel','The clock bias hides in every apparent delay','One acquisition cannot separate b from the unknown path delays.',[
 ...panel('multipath-mean',96,190,1088,178,'THE UNKNOWN MULTIPATH MEAN',M(R`\mathbf M_k=\sum_{\ell=1}^{P}\alpha_\ell\,e^{-j2\pi f_k(\tau^{\mathrm g}_\ell+b)}\,\mathbf a_r(\mathbf q_{r,\ell})\,\mathbf a_t(\mathbf q_{t,\ell})^H\mathbf X`),{size:23,fill:C.cool}),
 ...panel('gauge',96,391,532,248,'A DIRECTION THE DATA CANNOT SEE',M(R`\partial_b\mathbf M_k=\sum_{\ell}\partial_{\tau^{\mathrm g}_\ell}\mathbf M_k`)+'Move b by δ and every '+I(R`\tau^{\mathrm g}_\ell`)+' by −δ. No '+I(R`\mathbf M_k`)+' changes, so J is singular in that direction.',{size:21}),
 ...panel('apparent',652,391,532,248,'WHAT THE CHANNEL DETERMINES',M(R`\tau^{\mathrm a}_\ell=\tau^{\mathrm g}_\ell+b`)+'Apparent delays, AoA, AoD and gains are the channel parameters.<br><br>Differences '+I(R`\tau^{\mathrm a}_\ell-\tau^{\mathrm a}_1`)+' are clock-free.',{size:21,fill:C.mint})
],'The clock factor exp(-j2pi f_k b) multiplies the whole received tone, so its derivative equals the sum of all geometric-delay derivatives. With free path gains and free geometric delays, the Fisher matrix in (tau_g, b) has the null vector (-delta, ..., -delta, +delta). Reparameterizing to apparent delays removes the null direction, which is why the coded-pilot lab reports apparent delay. Free complex gains also absorb any common phase. A delay prior or a clock reference would be additional information and must be written into the model.',['Model','Li']);

two('clock-geometry','Unknown channel','Geometry, not the channel, separates the clock','Every apparent delay shares one b. The paths tie it to the unknown UE pose and map.',
 'LINE OF SIGHT ALONE',M(R`c\,\tau^{\mathrm a}_1=\|\mathbf x-\mathbf p_{\mathrm{BS}}\|+c\,b`)+'UE unknowns: 3 position, 3 orientation, 1 clock. LoS supplies 5 measurements.<br><br>AoD fixes the ray from the known BS. Along that ray, a longer range and an earlier clock give the same '+I(R`\tau^{\mathrm a}_1`)+'.<br><br><b>b is not identifiable.</b>',
 'ADD ONE SINGLE-BOUNCE PATH',M(R`c\,\tau^{\mathrm a}_2=\|\mathbf v-\mathbf p_{\mathrm{BS}}\|+\|\mathbf x-\mathbf v\|+c\,b`)+'The incidence point v is unknown: 5 new measurements, 3 new unknowns. Now 10 and 10.<br><br>The bounce AoA fixes the UE roll and the ratio of ray lengths. The delay difference sets the scale.<br><br>For generic geometry '+I(R`\mathbf H^T\mathbf J_\eta\mathbf H`)+' becomes full rank and b has a finite bound.',
 'In 3D the UE has 3 position, 3 orientation and 1 clock unknowns. The LoS path gives 5 measurements. With one single-bounce path: the two AoDs fix the rays from the BS to the UE and to v; the LoS AoA fixes UE orientation up to roll about the LoS axis; the bounce AoA fixes that roll and the ratio of the two ray lengths; the delay difference sets the scale. Then b equals tau_1^a minus range over c. Counting is necessary, not sufficient: degenerate placements, such as v on the BS-UE line, lose rank. The bound on b comes from inverting the geometric FIM after mapping every path to the common unknowns, as on the radio SLAM slide.',['Shah','Model'],[21,21]);
...
add('worked-delay','06 · worked example','A numerical delay bound','The teaching example assumes Γ = 1000, corresponding to 30 dB aggregate SNR.',[
 ...
 ...panel('distance',652,389,532,241,'APPARENT PATH LENGTH',M(R`c\,\sigma_{\tau,\rm LB}=9.24\,\mathrm{mm}`)+ 'This bounds '+I(R`c\,\tau^{\mathrm a}`)+', which still contains '+I(R`c\,b`)+'. It becomes a path-length bound only after geometry resolves the clock.',{size:22,fill:C.mint})
...
two('slam','11 · connection to radio SLAM','Channel bounds become geometric information','Use the same propagation model as the radio SLAM factor.',
 ...
 'THE MODEL DETERMINES THE INFORMATION','AoA constrains the last segment. AoD constrains the first segment. Delay constrains total length plus c b.<br><br>Sum independent link information after mapping every link to common unknowns.<br><br>Eliminate nuisance variables and fix the coordinate gauge before reading pose bounds.',
...
add('references','References and further reading','Sources and modeling assumptions','The live values come from the displayed model and its derivatives.',[
 ...
 txt('assumptions',96,566,1088,70,'<b>Model:</b> Known pilots and an unknown channel. One isolated static path with unknown delay, angles and complex gain. White noise, calibrated arrays. The two-path lab explicitly holds delays known.',{fontSize:19,color:C.muted})
